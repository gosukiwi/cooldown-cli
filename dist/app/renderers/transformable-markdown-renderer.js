...
exports.TransformableMarkdownRenderer = class extends MarkdownRenderer {
  ...
  run(transformation, node, entering, done) {
    ...
    if (typeof transformation[node.type][action] === 'function') {
      return transformation[node.type][action].call(this, node, function() {
        return done(true);
      });
    } else {
      return done(false);
  ...
  cleanup(done) {
    return async.each(this.transformations, (transformation, _done) => {
      if (typeof transformation.finally === 'function') {
        return transformation.finally.call(this, _done);
      } else {
        return _done();
      }
    }, function(err) {
